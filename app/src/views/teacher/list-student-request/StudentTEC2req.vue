        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title mb-2">
                    ข้อมูลนักศึกษาชั้นปริญาตรีชั้นปีที่ 2 (ผู้ขออนุมัติ)
                    <div>
                        <router-link :to="`/teacher-index/student-tec2req`">
                            <button class="btn btn-primary m-1">ขออนุมัติ</button>
                        </router-link>
                        <router-link :to="`/teacher-index/student-tec2active`">
                            <button class="btn btn-warning m-1">กำลังฝึก</button>
                        </router-link>
                        <router-link :to="`/teacher-index/student-tec2success`">
                            <button class="btn btn-success m-1">ฝึกจบแล้ว</button>
                        </router-link>
                    </div>
                </div>
                <table class="table">
...
const fetchData = async () => {
    try {
        ...
        users.value = response.data.filter(user => user.status === "Request training" && user.year === "ป.ตรี ปีที่ 2");
    } catch (error) {
        Swal.fire({
            title: "error",
