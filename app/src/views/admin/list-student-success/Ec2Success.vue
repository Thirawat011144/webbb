...
const fetchData = async () => {
    try {
        ...
        users.value = response.data.filter(user => user.status === "Finished training" && user.year === "ป.ตรี ปีที่ 2");
    } catch (error) {
        Swal.fire({
            title: "error",
    ...
    <section class="content">
        <div class="card">
            <div class="card-header">
                <div class="card-title mb-2">ข้อมูลนักศึกษาชั้นปริญาตรีชั้นปีที่ 2 (ฝึกจบแล้ว)
                    <div>
                        ...
                        <router-link :to="`/admin-index/Ec2-success`"> <button
