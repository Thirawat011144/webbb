...
const fetchData = async () => {
    try {
        ...
        users.value = response.data.filter(user => user.status === "Finished training" && user.year === "ป.ตรี ปีที่ 4");
    } catch (error) {
        Swal.fire({
            title: "error",
    ...
    <section class="content">
        <div class="card">
            <div class="card-header">
                <div class="card-title mb-2">ข้อมูลนักศึกษาชั้นปริญาตรีชั้นปีที่ 4 (ฝึกจบแล้ว)
                    <div>
                        ...
                        <router-link :to="`/admin-index/Ec4-success`"> <button
                                class="btn btn-success m-1">ฝึกจบแล้ว</button> </router-link>
                    </div>
                </div>
                <table class="table">
                    <thead>
                    ...
                    <tbody>
                        <tr v-for="(user, index) in sortedUsers" :key="user.id">
                            ...
                            <td>{{ user.branch }}</td>
                            <td>{{ user.year }}</td>
                            <td class="text-center">{{ user.companyDetails ? user.companyDetails.companyName : 'Null' }}
                            </td>
                            <td>
