    ...
    try {
        ...
        if (result.isConfirmed) {
            const updatePromises = users.value.map(user => {
                return axios.put(`${config.api_path}/user/${user.id}`, { status: 'เสร็จสิ้น', year:'ปวส 1' });
            });
            ...
            <div class="card-header">
                <div class="card-title mb-2">ข้อมูลนักศึกษาชั้นประกาศนียบัตรวิชาชีพ ชั้นปีที่ 3 (ผ่าน)
                    <div>
                        ...
                        <button class="btn btn-info m-1" @click="downloadExcel">การประเมินจากสถานประกอบการณ์</button>
                        <button class="btn btn-info m-1"
                            @click="downloadExcelUniversity">การประเมินจากมหาวิทยาลัย</button>
