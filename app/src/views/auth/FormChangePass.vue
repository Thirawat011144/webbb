...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (response.data.message === "Password has been reset successfully") {
            ...
            } else if (response.data.message === "Password has been reset successfully" && checkRole === 'teacher'){
                router.push('/teacher-index');
            } else if (response.data.message === "Password has been reset successfully" && checkRole === 'admin'){
                router.push('/admin-index');
            } else {
                // มี token (กรณีเปลี่ยนรหัสผ่านหลังจากล็อกอิน)
